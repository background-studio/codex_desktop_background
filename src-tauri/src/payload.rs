...
pub fn build_active_payload_from_bytes(
    ...
    Ok(ActivePayload {
        script,
        revision: payload_revision,
        media_bytes: Arc::from(bytes.to_vec()),
...
pub fn build_active_payload(
    ...
    build_active_payload_from_bytes(&bytes, mime_type, kind, display)
}
...
mod tests {
    ...
    fn keeps_large_media_out_of_cdp_script() {
        let root = std::env::temp_dir().join(format!("codex-large-payload-{}", Uuid::new_v4()));
